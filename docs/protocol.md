...
广播名的形态是 **`<type>-<id>`**,例如 `m5paper-c119cc`:

- `type` 由固件的 `LinkConfig::deviceType` 给,同一类设备共用
- `id` 缺省从 **efuse MAC 后三字节**派生 —— 同一份固件烧多块板子自动不重名,
  不用为了区分设备去改常量重编,那是多设备场景最容易出错的一步

中枢按 `<type>-` 前缀发现同类设备,按全名认具体哪一台。

> 身份为什么塞在名字里而不是 manufacturer data:BLE 广播包只有 31 字节,一个
> 128 位 service UUID 就吃掉 18 字节,再塞厂商数据基本没地方(名字本身也是靠
> scan response 才装得下)。塞名字零额外开销,而且人眼可读。
...
```json
{"t":"hello","id":"c119cc","type":"m5paper","name":"m5paper-c119cc",
 "fw":40,"mtu":517,"caps":"usage,ev,cmd"}
```

`caps` 让中枢知道这台设备吃哪些消息 —— 广播时可以跳过不认识该消息的设备。

⚠️ 这是**框架自己的协议**,由 `BleHub` 消化掉,**不往应用层转发**。
这是「框架只管传输、不碰内容」原则里少数几个刻意的例外,所以边界要钉死:
`BleLink` 仍然是纯哑管道,只有 hub 这一层认得 hello。

⚠️ 设备没上报过 `caps` 时,中枢**按「什么都支持」处理** —— 宁可多发一条它会
忽略的消息,也不要因为握手还没到就静默漏发。

### 路由在主机侧,消息里没有 target 字段

每条 BLE 链路本来就是点对点的:hub 按 target 选中哪个链路,消息发出去时对端
只有一个。`broadcast` 就是 for-each-link 发一遍。所以固件只需要能**报出自己是谁**,
不需要在消息里带路由字段,也不需要自己做过滤。
