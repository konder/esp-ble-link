...
<a id="c10"></a>
### C10. ★★★ 两个**同 bundle id** 的进程共存,后起的那个扫描一台都扫不到

**这条推翻了这个框架的一条地基假设**,而且它自己也被订正过一次 —— 两版都记在这里,
因为「错的成因」比「没有成因」更贵:它会把人引向没用的修法。

原来的地基(写在 `hub.py` 里)是:

> 一个 bundle → 一次 TCC 授权;N 个实例 → N 个进程 → N 个独立 central。两个好处都要得到。

**假的。** 当初的验证只看到「两个实例都拿到 `central_state: 5`」就下了结论 ——
两边都能"开机",但**只有一个收得到扫描结果**。

**症状**:另一个同 id 的进程(这里是常驻菜单栏)开着的时候,
- worker 扫描**一个 `didDiscover` 都收不到**,必然超时 → 自杀 → 重启 → 再超时……
  从外面看像**链路在疯狂 flapping**(`c/d` 每十几秒涨一次),极易误判成链路层的 bug;
- **已建立的连接完全不受影响**(实测菜单栏开着 150 秒,在线链路 `disconnected` 为 0),
  `bluetoothd` 也照常在收广播 —— 所以完全想不到是某个进程干的;
- 不同 bundle id 的 helper 之间**没有**这个问题(两根 stick 的 helper 与 worker 长期共存正常)。

**第一版成因是错的:**「只要**持有** CBCentralManager 就会」。
按这个结论做的修法是「菜单栏改成按需创建 central,配好之后一个都不建」——
...
**解**:**一个 bundle id 同时只跑一个进程。**
- 菜单栏(只读文件显示器)用**独立的 bundle id** —— `build-helper --install` 会把装到
  `~/Applications` 的那份改成 `<id>.monitor`。它不碰 CoreBluetooth,所以不需要蓝牙授权。
- 多设备**不能**靠「一个 bundle 起 N 个 worker」实现。要么每台设备一个 bundle id
  (代价:每个 id 都要人点一次授权框),要么**一个进程管 N 个 peripheral**
  (CoreBluetooth 本来支持;代价是放弃「坏状态是进程级、丢进程就能恢复」那条隔离)。
- 首次授权:双击**worker 那份** bundle → 菜单里「请求蓝牙授权」→ **授完就退出它**
  (它和 worker 同 id,开着就会掐死 worker 的扫描)。

> 副作用要接受:菜单栏不建 central 就**不知道**蓝牙授权状态,菜单里得老实写「未查询」,
...
**症状**:某个长驻进程(这里是菜单栏实例)在跑,于是
- 任何新起的 central **一个 `didDiscover` 都收不到**,扫描必然超时;
- 但**已建立的连接全都正常**(键盘、鼠标、别的 stick 该连的还连着);
- 而且 `bluetoothd` 明明在收广播 —— `system_profiler SPBluetoothDataType` 的
  「Not Connected」列表里 RSSI 是新鲜的。

**判别方法(一条命令定性)**:把那个长驻进程杀掉,立刻重跑**同一条**扫描命令。
```
espble scan --app <bundle> --scan-timeout 25     # 之前:一台都扫不到
kill <长驻进程 pid>
espble scan --app <bundle> --scan-timeout 25     # 之后:立刻 6 台
```
差别只有"那个进程在不在",就是它。

**成因**:菜单栏实例为了省电改成了「扫 15s / 停 45s」的占空比,于是每分钟
`scanForPeripherals` + `stopScan` 各一次。macOS 的扫描参数是**全机聚合**的,
这种周期性 churn 会把聚合状态搅坏(日志里能看到
`Server.LE.Scan … active 0 / needToRestart=0`),结果**所有**客户端都收不到广播。

**最反直觉的地方:它把自己要显示的那条链路弄断了。** 菜单栏开着,
Python 侧的 worker 就永远连不上设备;而菜单栏显示的正是"设备未发现"——
一个自己造成、又自己如实报告的故障。

**解**:**能不扫就不扫。** 菜单栏是个只读的文件显示器,链路状态的权威来源是
worker 写的 `events.jsonl` 和注册表,不是扫描。只有「压根没有会话文件可读」的
单设备场景才扫,而且是**一次性、开着不动**的连续扫描(`AllowDuplicates: false`),
绝不周期性 stop/start。

> ⚠️ 这条和 [C8](#c8) 是**一对相反的错误**,两个都真实发生过:
> C8 是"扫太多"(`AllowDuplicates: true` 连扫 9 天,抢射频拖累别人),
> C9 是修 C8 时引入的"开开停停"(更严重,直接让所有人扫不到)。
> 中间那条正确的路很窄:**要么一直开着不动,要么压根不扫** —— 别在中间找平衡。
